...
- Version set to 0.4.0, the upstream release that actually carries this fix (`git tag --contains`
  on the upstream commit resolves to `libs/langgraph-checkpoint-mongodb/v0.4.0`; upstream 0.3.0 and
  0.3.1 predate it, despite what the GHSA affected-version table says). This fork is not otherwise
  feature-equivalent to upstream 0.4.0; the number tracks the security patch level only.
